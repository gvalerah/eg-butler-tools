...
class nutanix_prism_vm(db.Model,Serializer):
    ...
    vm_drp_remote    = db.Column( db.Boolean, default=0 )
...
class Role(db.Model,Serializer):
    ...
    def insert_roles():
        roles = {
            ...
            '''
            'Solicitante': (   Permission.CUSTOMER, False),
            'Aprobador': (   Permission.VIEW |
                            Permission.REPORT |
                            Permission.EXPORT, True),
            'Visualizador': (    Permission.VIEW |
                            Permission.DELETE |
                            Permission.MODIFY |
                            Permission.REPORT, False),
            'Administrador':    (0xfe, False),                      # Administrator does not have 'Customer' permisions
            'Auditor':          (0x1fe, False),                      # Auditor does not have 'Customer' permissions
            'God':              (0xfff, False)
        }
        for r in roles:
...
class User(UserMixin, db.Model,Serializer):
    ...
    ldap          = db.Column( db.Boolean, default=0 )
    ldap_method   = db.Column( db.String(45) )
    ldap_user     = db.Column( db.String(45) )
    ldap_common   = db.Column( db.String(45) )
    ldap_host     = db.Column( db.String(45) )
    ldap_port     = db.Column( db.Integer, default=0 )
    ldap_domain   = db.Column( db.String(45) )
    ...
    # source: EG-Suite-Tools/Butler/code/src/include/models/flask_users_methods.py
