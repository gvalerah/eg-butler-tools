...
if (os.path.isfile(config_file)):
        ...
        flask_port    = config_ini.getint('General','flask_port'   ,fallback=5010)
        gunicorn_host = config_ini.getint('General','gunicorn_host',fallback='0.0.0.0')
        gunicorn_port = config_ini.getint('General','gunicorn_port',fallback=8010)
else:
    sys.exit(1)

from    app                 import create_app,db,logger
C = Context("Butler Web Server",config_file,logger)
C.Set()
...
if __name__ == '__main__':
    ...
    from    emtec.butler.db.flask_models    import User, Role
