...
from core import create_app,db,logger
# **********************************************************************
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            #wait_seconds = egconfig.getint('Deamon','pool_seconds',fallback=60)
            wait_seconds = app.config.get('CORE_POOL_SECONDS',59)
            logger.info('---------------------------------------------')
            logger.info(f'EG Butler CORE prober will wait for {wait_seconds} seconds ...')
            print      (f'EG Butler CORE prober will wait for {wait_seconds} seconds ...')
            time.sleep (wait_seconds)
    except KeyboardInterrupt:
        logger.warning(f'EG Butler CORE prober Interrupted.')
        logger.warning(f'EG Butler CORE prober cleaning UP ...')
        # Ay Tidy up process here
        logger.warning(f'EG Butler CORE prober God Bye.')
        logger.warning('----------------------------------------------')
        sys.exit(1)
    except Exception as e:
        logger.critical(f'EG Butler CORE prober EXCEPTION: {str(e)}')
        raise e
